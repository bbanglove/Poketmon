...
public class TSP {
	...
	private ArrayList<ArrayList<Integer>> W= new ArrayList<ArrayList<Integer>>(); // weight 그래프 정보
	private int[][] dp; // 최소비용 누적값 [a][b] : a는 현재 위치, b는 visited 정보 비트마스크
								// 예를들어 dp[1][7] 은 현재 위치 1번정점에서 111(=7) 즉 1,2,3번
								// 도시 다 방문했을때의 최소비용 저장
	private  List<Integer> path;
	private  int N; // 정점 갯수
	private final int INF = 1000000000; // 리턴값 default
	private long distance; // 최저비용 저장
	private int last;
		...
		for (int i = 0; i < N; i++) {
			...
			for (int j = 0; j < N; j++) {
				if(weight.get(i).get(j).getWeight() == -1){
					...
					eRow.add(weight.get(i).get(j).getWeight()); // 임시 어레이리스트에 가중치 추가
				}
			}
			W.add(eRow);
		}
	...
	public List<Integer> getVertexNum(int distance) // 경유 노드 저장
	{
		...
		for (int i = 0; i < N; i++) {
			...
			for (int k = 0; k < N; k++) {
				...
				if (W.get(piv).get(k) == 0) // 0은 경로가 없으므로 pass
					continue;
...
*/
	...
	private int getShortestPath(int current, int visited) {
		...
		for (int i = 0; i < N; i++) {
			...
			if (W.get(current).get(next) == 0 || W.get(current).get(next) == -1) // 0은 경로가 없으므로
																	// pass
				continue;
